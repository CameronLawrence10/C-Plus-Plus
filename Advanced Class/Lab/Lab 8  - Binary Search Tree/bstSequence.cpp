 ...
 * Lab Section: 003
 * Assignment Name: Best Sequence Search Tree
 */
...
//if nothing is in tree create a new node    
  if(!T) return new Node(v);
  int rr = T -> left ? T -> left -> size : 0;
//when to the left side of the tree 
  if(r <= rr) {
    T -> left = insert( T-> left, v, r);
  }
//else to the ride side of the tree 
    else { 
      T -> right = insert(T -> right, v, r - rr - 1);
    }
//adjust size of tree  
  fix_size(T);
  return T;
...
//almost same as insert just split instead, for left and the right side of the tree  
  assert(r >= 0 && r<= T->size);
  int rr = T -> left ? T -> left -> size : 0;
  ...
  if (rr >= r) {
    ...
    *R = T;
      ...
      split(T -> right, r - rr - 1, &T -> right, R);
      *L = T;
    }
//then adjust and fix size  
  fix_size(T);
}
...
//split T and create a new node called begin for 1/n
  if (rand() % (T -> size + 1) == 0)  {
    Node *begin = new Node(v);
    split(T, r, &begin -> left, &begin -> right);
    fix_size(begin);
    return begin;
  }
  assert( T -> size >= r && 0 <= r);

//same as split and insert 
  int rr = T -> left ? T -> left -> size : 0;
//insert vr to the left   
  if (rr >= r) {
    T -> left = insert_random(T -> left, v, r);
    fix_size(T);
  }
//insert vr to the right    
    else {
      T -> right = insert_random(T -> right, v, r - rr - 1);
      fix_size(T);
    }
...
Node *order_n_teams(int n)
{
  ...
  for (int i=1; i<n; i++) {
    ...
    if (did_x_beat_y(i, select(T,0)->key)) // can we insert at beginning?
      ...
      // binary search to find the first team that team i beat
      // set low to 0 and high to T->size-1
	    int low = 0;
      int high = T->size-1;
    
      while (low < high) {
        // set mid to the middle of low and high
        int mid = (low+high)/2;
        // if team i beat the team at rank mid, set low to mid+1 else set high to mid
        if (did_x_beat_y(select(T, mid)->key, i)) low = mid+1;
        else high = mid;
      }
      // insert team i at rank low
      T = insert_random(T, i, low);
    }
  }
  return T;
}
